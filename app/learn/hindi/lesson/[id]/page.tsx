...
export default function HindiLessonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    params.then(({ id }) => {
      ...
      const foundLesson = getLesson("hindi", id_num)
      if (foundLesson) {
  ...
  return <HindiLessonContent lesson={lesson} lessonId={lessonId} />
}

// Client component to handle state and interactivity
function HindiLessonContent({ lesson, lessonId }: { lesson: any; lessonId: number }) {
  ...
  const saveXpToDatabase = async (xpEarned: number) => {
    if (xpEarned <= 0) return; // Prevent any update if no XP earned
    setIsSaving(true)
    try {
      const {
        data: { user },
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {completed && earnedXp === 0 ? (
          <div className="max-w-2xl mx-auto text-center">
            <div className="bg-red-500 rounded-3xl p-12 text-white shadow-2xl mb-8">
              ...
              <div className="flex flex-col gap-3 mt-8">
                <Button
                  onClick={() => router.push("/learn/hindi")}
          ...
          <div className="max-w-2xl mx-auto text-center">
            <div className="bg-gradient-to-br from-blue-400 to-blue-600 rounded-3xl p-12 text-white shadow-2xl mb-8">
              <div className="text-7xl mb-4">🎉</div>
              <h2 className="text-4xl font-bold mb-4">Lesson Complete!</h2>
              <p className="text-2xl mb-6">{earnedXp} XP Earned</p>
              <p className="text-blue-100 mb-6">Great job learning {lesson.title}!</p>
              {isSaving && <p className="text-blue-100 text-sm">Saving your progress...</p>}

              <div className="flex flex-col gap-3 mt-8">
                <Button
                  onClick={() => router.push("/learn/hindi")}
                  className="bg-white text-blue-600 hover:bg-gray-100 font-bold py-6"
                  disabled={isSaving}
                >
                  Back to Dashboard
                </Button>
                <Button
                  onClick={() => router.push(`/learn/hindi/lesson/${lessonId + 1}`)}
                  className="bg-white text-blue-600 hover:bg-gray-100 font-bold py-6"
                  disabled={isSaving}
                >
                  Next Lesson
          ...
          <>
            ...
            {!learningCompleted ? (
              <div className="max-w-2xl mx-auto">
                <div className="bg-gradient-to-br from-blue-100 to-blue-50 rounded-2xl p-8 border-2 border-blue-300 mb-8">
                  <h2 className="text-2xl font-bold text-blue-900 mb-4">Lesson Content</h2>
                  ...
                  <Button
                    ...
                    className="w-full mt-6 bg-blue-600 hover:bg-blue-700 text-white font-bold py-6"
                  >
